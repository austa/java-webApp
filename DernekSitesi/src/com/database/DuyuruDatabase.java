...
public class DuyuruDatabase {
	...
	public boolean duyuruEkleme(String baslik, String icerik,String resim) throws ClassNotFoundException, SQLException {
		Statement statement = connectDatabase();
		int kontrol =statement.executeUpdate("INSERT INTO duyuru(`baslik`, `icerik`, `resim`) VALUES ('" +baslik+ "','" +icerik+ "','" +resim+"')");
	...
	public Duyuru duyuruGoruntule(String id) throws ClassNotFoundException, SQLException {
		...
		while (result.next()) {
			...
			duyuru.setResim(result.getString("resim"));
		}
		return duyuru;
	}
	public List<Duyuru> tumDuyurular() throws ClassNotFoundException, SQLException {
		...
		while (result.next()) {
			Duyuru duyuru = new Duyuru(result.getString("id"), result.getString("baslik"), result.getString("icerik"), result.getString("zaman"), result.getString("resim"));
	...
	public List<Duyuru> tumDuyurular(int baslangic,int limit) throws ClassNotFoundException, SQLException {
		List<Duyuru> baslikListesi = new ArrayList<Duyuru>();
		Statement statement = connectDatabase();
		ResultSet result = statement.executeQuery("SELECT * FROM `duyuru` order by id desc limit " + baslangic + "," + limit);
		while (result.next()) {
			Duyuru duyuru = new Duyuru(result.getString("id"), result.getString("baslik"), result.getString("icerik"), result.getString("zaman"), result.getString("resim"));
			baslikListesi.add(duyuru);
		}
