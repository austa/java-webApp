...
public class HaberDatabase {
	...
	public boolean haberEkleme(String baslik, String icerik,String resim) throws ClassNotFoundException, SQLException {
		Statement statement = connectDatabase();
		int kontrol =statement.executeUpdate("INSERT INTO haber(`baslik`, `icerik`, `resim`) VALUES ('" +baslik+ "','" +icerik+ "','" +resim+"')");
	...
	public Haber haberGoruntule(String id) throws ClassNotFoundException, SQLException {
		...
		while (result.next()) {
			...
			haber.setResim(result.getString("resim"));
		}
		return haber;
	}
	public List<Haber> tumHaberler() throws ClassNotFoundException, SQLException {
		...
		while (result.next()) {
			Haber haber = new Haber(result.getString("id"), result.getString("baslik"), result.getString("icerik"), result.getString("zaman"), result.getString("resim"));
	...
	public List<Haber> tumHaberler(int baslangic,int limit) throws ClassNotFoundException, SQLException {
		List<Haber> baslikListesi = new ArrayList<Haber>();
		Statement statement = connectDatabase();
		ResultSet result = statement.executeQuery("SELECT * FROM `haber` order by id desc limit " + baslangic + "," + limit);
		while (result.next()) {
			Haber haber = new Haber(result.getString("id"), result.getString("baslik"), result.getString("icerik"), result.getString("zaman"), result.getString("resim"));
			baslikListesi.add(haber);
		}
